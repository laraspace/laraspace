<div class="skin-tools">
    <a class="skin-tools-action">
        <i class="fa fa-cog"></i>
    </a>
    <div class="skin-tools-content">
                    <h5 class="mt-2">Select Skin</h5>
                    <div class="row mt-4">
                        <div class="col-sm-6">
                            <a href="#" class="skin-radio active" data-skin="default" title="Skin - Default">
                                <img src="/assets/admin/img/skins/skin-default.png" class="img-fluid">
                            </a>
                        </div>
                        <div class="col-sm-6">
                            <a href="#" class="skin-radio" data-skin="tyrell" data-logo="white" title="Skin - Tyrell">
                                <img src="/assets/admin/img/skins/skin-tyrell.png" class="img-fluid">
                            </a>
                        </div>
                    </div>
                    <div class="row mt-4">
                        <div class="col-sm-6">
                            <a href="#" class="skin-radio" data-skin="arryn" data-logo="white" title="Skin - Arryn">
                                <img src="/assets/admin/img/skins/skin-arryn.png" class="img-fluid">
                            </a>
                        </div>
                        <div class="col-sm-6">
                            <a href="#" class="skin-radio" data-skin="lannister" data-logo="white" title="Skin - Lannister">
                                <img src="/assets/admin/img/skins/skin-lannister.png" class="img-fluid">
                            </a>
                        </div>
                    </div>
                    <div class="row mt-4">
                        <div class="col-sm-6">
                            <a href="#" class="skin-radio" data-skin="stark" title="Skin - Stark">
                                <img src="/assets/admin/img/skins/skin-stark.png" class="img-fluid">
                            </a>
                        </div>
                        <div class="col-sm-6">
                            <a href="#" class="skin-radio" data-skin="targaryen" title="Skin - Targaryen">
                                <img src="/assets/admin/img/skins/skin-targaryen.png" class="img-fluid">
                            </a>
                        </div>
                    </div>
    </div>
</div>